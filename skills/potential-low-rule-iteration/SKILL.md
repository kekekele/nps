...
每轮基线诊断后，必须按以下优先级决定动作，不得默认新增规则：

| 优先级 | 触发信号 | 首选动作 | 处理方式 |
| --- | --- | --- | --- |
| P0 | 字段缺失、类型/日期异常、分类口径变化、规则命中率异常漂移 | 修复数据或配置 | 修正画像抽取、字段映射或时间边界；在问题解决前不调权重、不新增规则 |
| P1 | FP 集中于某条现有规则，且规则业务语义仍成立 | 修改规则 | 收紧 `actions`、`patterns`、`within_days` 或 `min_count`；必要时改为 `all_of` 组合条件 |
| P2 | 现有规则收紧后仍导致高 FP、长期优化为 0 权重、命中量不足或没有边际贡献 | 禁用规则 | 先在 `weight_candidates` 中保留 `0`；连续多个批次无贡献后设置 `enabled: false` |
| P3 | 仍存在集中的 FN 缺口，且没有相近现有规则可通过修改覆盖 | 新增规则 | 从 FN 与 TN 的结构化差异中提出候选，使用 `weight_candidates: [0, 1, 2]` 灰度加入 |
| P4 | 指标稳定且没有满足准入条件的新模式 | 不改规则 | 积累更多标签，持续观察数据与规则命中漂移 |

当 FN 与 FP 同时偏高时，先完成 P1/P2，减少弱规则导致的噪声；再执行 P3，从剩余 FN 中寻找未覆盖、可解释的新模式。每轮只能执行一项 P1、P2 或 P3 的改动，保证指标变化可归因。
...
每次实验创建新的 YAML 版本和输出目录。先按“迭代动作优先级”选择动作；每轮只改变一个维度：

1. 按 P1 修改一条现有规则、按 P2 禁用一条现有规则，或仅在满足 P3 时新增一条候选规则。
